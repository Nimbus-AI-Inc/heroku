...
def calc_main():
    ...
    **Please keep in mind that this app may take anywhere from 15 seconds to a couple minutes to load due to its large slugsize.**
    ...
    2) Next, selct a **value of n**, which is the exponential degree we will set our quadratic regression function to (basis for the LTSTM Neural Network's training model). We recommend setting n to a higher value such as 7 or 8 for more accuaracy. 
