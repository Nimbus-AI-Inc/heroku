...
def calc_main():
    ...
    **Please keep in mind that this app may take anywhere from 15 seconds to a couple minutes to load due to its large slugsize.**
    ...
    1) Select the **Ticker Symbol** located within the sidebar on the left to determine a prediction of the stock value for the next ten days, that is generated by our AI algo. 

    2) Next, selct a **value of n**, which is the exponential degree our polynomial regression function will be set on (basis for the LTSTM Neural Network's training model). We recommend setting n to a higher value such as 7 or 8 for more accuaracy. 

    3) View both our charted prediction on the **graph below**, or the individual values for the stock over the next 10 days shown in the **table** located in the sidebar. 
    """)
